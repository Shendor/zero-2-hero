...
Liveness probes are for situations when an app has crashed or isn't responding anymore. Just like the readiness probe, a liveness probe can be used to perform a set of health checks.
...
As a simple example here is a health for a Go application.
...
We will now move on to a tutorial of what we have just seen.
...
```
The command will stay open to host the port forwarding. 
Hit ctrl-Z, and then immediately run `bg` to move the port-forwarding command into the background. 
...
Delete the healthy-monolith deployment:
```
