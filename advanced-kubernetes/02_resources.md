...
* Kubernetes looks for a suitable Node with enough resources.
...
* A Pod will not be deployed if there is no suitable Node with enough capacity.
* Requested resources can be exceeded (with _potential_ consequences).
* Guideline to resource usage within a Pod
  - (or minimum needed resources)
...
A Limit is ignored in the scheduling phase of the Pod lifecycle.
 ...
 - Hard limits impose immediate effect when exceeded
...
* Excess CPU resources will be distributed as a ratio of the CPU requested.
...
* Pods get the amount of memory they request.
* If they exceed their request, they _may_ be killed.
  * if the node starts running out of memory
* If Pods consume less memory than requested, they will not be killed
  * Except in unusual QoS scenarios
...
* If the system runs out of resources, what to kill first?
...
* `limits` is non-zero, and set on all containers in pod
* `requests` is equal to limits (or not set)
...
### How is QoS enforced?
* CPU: Best Effort or Burstable containers using more than request are throttled.
    * CPU shares + CPU quota.
* Memory: Best Effort/Burstable containers using more than request are killed.
...
---
 ...
 - Let's demonstrate this with a simple container that will consume as much CPU as possible.
```
...
---
 ...
 - You can use `kubectl top pod <PODNAME>` to view CPU usage.
...
```
 ...
 - _Note_: it can take a moment for the top output to reflect the changes to your deployment.
...
```
 ...
 - Each container is getting an equal share of CPU and Kubernetes is not
...
### Memory requests
 ...
 - By default, a container is able to consume as much memory on the node as possible. 
    - Should specify the amount of memory your container will require to run.

 - Let's demonstrate this by creating a Pod that runs a single container which requests 100Mi of memory. 
 - The container will allocate and write to 200MB of memory every 2 seconds.
...
```
 ...
 - We request 100Mi, but have _burst_ our memory usage to a greater value.
...
Don't do this on your VM it will crash!
...
---
 ...
 - Your trainer should have provided you with some code we will use in the
   following example.
